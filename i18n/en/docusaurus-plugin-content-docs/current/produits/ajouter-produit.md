---
sidebar_position: 4
title: Add a product
---

# Add a product

Add products to your project from different sources.

## Adding methods

### From the library

Add products already in your catalog:

1. In your project, go to the **Products** section
2. Click **Add from library**
3. Use filters to find the desired products
4. Check the products to add
5. Click **Add to project**

Products are added with their full information (price, images, descriptions, etc.).

### Create a new product

Create a product directly in the project:

1. In the project **Products** section, click **New product**
2. Fill in the product information:

#### Basic information
- **Name**: Product name (required)
- **Reference**: Internal or manufacturer reference
- **Product type**: Product categor(ies) — manage the list in [Product types](/agence/types-produits)
- **Description**: Detailed description
- **Technical note**: Internal notes (never shown to clients; exportable in the booklet and spreadsheet)

#### Commercial information
- **Manufacturer**: Manufacturer name
- **Supplier**: Supplier name
- **Unit price**: Price per unit
- **Quantity**: Quantity in the project

#### Organization
- **Area**: Project area (room, floor, etc.) — see [Area management](/projet/gestion-zones)
- **Lot**: Associated trade category — see [Managing lots](/projet/gestion-lots)
- **Supply / installation**: Visible only when a lot is selected
  - **Supply & install**
  - **Install only**
  - **Neither supply nor install**
- **Status**: Lifecycle stage (see [Product statuses](/comprendre/statuts-produits))

#### Delivery tracking
These fields are visible to the client when filled in:

- **Estimated reception date**
- **Tracking link** (carrier URL)
- **Tracking number**

When a tracking link is set, **Track shipment** opens tracking in a new tab.

3. Add images if needed:
   - Click **Add images**
   - Select files from your computer
   - Drag and drop images or click to browse

4. Click **Create**

### From the Chrome extension

The Chrome extension lets you quickly add products from any website:

1. Install the Chrome extension (see [Installing the extension](/extension/installation))
2. Navigate to a website containing a product
3. Click the extension icon
4. The extension automatically extracts product information
5. Review and complete the information if needed
6. Select the destination project
7. Click **Add to project**

See the [extension documentation](/extension/utilisation) for more details.

## Edit a product

After adding a product, you can edit it:

1. Click the product in the list
2. Edit the desired information
3. Click **Save**

## Duplicate a product

To create a copy of an existing product:

1. Click the menu (⋮) next to the product
2. Select **Duplicate**
3. The product is copied with the "(copy)" suffix
4. Edit the copy as needed

## Tips

- Use the [catalog](/catalogue/gerer-catalogue) for recurring products
- Create products in the project for site-specific items
- Use the Chrome extension to quickly extract products from websites
- Always add images to make products easy to identify
