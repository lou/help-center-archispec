---
sidebar_position: 1
slug: /
title: Home
---

# Welcome to Archispec

Archispec is a comprehensive architectural project management platform that helps architecture professionals collaborate efficiently with their clients.

The application is available at [https://archispec.fr](https://archispec.fr).

## What is Archispec?

Archispec is a tool designed specifically for architects and interior designers who want to:

- **Manage their projects** in a centralized way
- **Collaborate with their clients** in real time
- **Source and organize** their product catalog
- **Generate exports** to streamline billing and site tracking
- **Plan their projects** visually and collaboratively

## Getting started

New to Archispec? Follow the guided path:

1. [Create your account](/premiers-pas/creer-compte)
2. [Set up your agency](/premiers-pas/configurer-agence)
3. [Create a project and add a product](/premiers-pas/premier-projet-produit)
4. [Share with a client](/premiers-pas/partager-client)

## Guides by topic

### Projects

- [Client sharing](/projet/partage-client) — Invite and collaborate with your clients
- [Section management](/projet/gestion-sections) — Organize by rooms and spaces
- [Managing lots](/projet/gestion-lots) — Trades and assignees
- [Planning](/projet/planning) — Track the build on a timeline
- [Files](/projet/fichiers) — Centralize documents and exports
- [Annotations](/projet/annotations) — Comment on a spot on a plan or PDF
- [Chat](/projet/chat) — Communicate internally or with the client
- [Notifications](/projet/notifications) — Stay informed of activity

### Products

- [Add a product](/produits/ajouter-produit) — Manual, catalog, or extension
- [Product statuses](/produits/statuts-produits) — Product lifecycle
- [Filter products](/produits/filtres) — Quickly find your selections
- [Select products](/produits/selection) — Bulk actions and exports
- [Price mode](/produits/mode-prix) — View price breakdowns
- [Purchase order](/produits/bon-de-commande) — Order from suppliers
- [Generate a 3D model](/produits/generation-3d) — 3D models via AI or import

### Catalog and extensions

- [Catalog](/catalogue) — Reusable product library
- [Browser extension](/extension/installation) — Source from any website
- [SketchUp extension](/sketchup/installation) — Place products in the 3D model

### Agency

- [Member management](/agence/gestion-membres) — Invite the team and manage roles
- [Companies](/agence/entreprises) — Contractor directory
- [Product types](/agence/types-produits) — Categories for filtering and organizing

### Billing

- [Subscription and limits](/facturation/abonnement-et-limites) — Free plan, pricing, and 3D credits
- [Referral program](/facturation/parrainage) — Credits for you and your referrals

### Client portal

- [Overview](/portail-client/vue-ensemble) — Your clients' dedicated space
- [Validate products](/portail-client/validation-produits) — Client validation workflow

### Reference

- [Glossary](/glossaire) — Archispec term definitions
- [FAQ](/faq) — Common issues and solutions

## Need help?

Use the search bar at the top of the page to quickly find a topic.
